...
char criptografia(char c) // Funcao para gerar um numero aleatorio e alterar o caractere conforme as regras
{
    if (numero(c))
    {
        return 'a' + (c - '0');
...
void criptografia_senha(char *senh) // Funcao para criptografar uma string
{
...
int carregar_user(struct User users[]) // Funcao para carregar os usuários de um arquivo
{
...
void salvar_user(struct User users[], int numero_de_usuarios) // Funcao para salvar os usuários em um arquivo
{
